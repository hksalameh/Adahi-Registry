...
import '@/lib/vfs_fonts.js'; //  <<<<----  تأكد أن هذا هو المسار الصحيح لملف الخطوط المخصص
 ...
 const generatePdfMakeDocument = (title: string, data: any[], columns: Array<{header: string, dataKey: string}>, fileName: string) => {
    if (!isAmiriFontConfigured) {
      toast({
        ...
        description: "لم يتم إعداد خط Amiri بشكل صحيح لـ pdfMake. لا يمكن إنشاء PDF. تأكد من وجود ملف vfs_fonts.js يحتوي على الخط.",
        variant: "destructive",
        duration: 7000,
      });
      return;
    ...
    const docDefinition: any = {
      ...
      content: [
        ...
        { text: exportDateText, style: 'subheader', alignment: 'right' as const, margin: [0, 0, 0, 20] },
        {
          table: {
            headerRows: 1,
            widths: pdfColumns.map(() => '*' as const), // توزيع متساوٍ مبدئيًا
